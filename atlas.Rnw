...
reg<-readOGR("C:/Users/User/Documents/SCF2016_FR/shapefiles",layer="ATLBioregions",verbose=FALSE)
par(mar=c(1,1,1,1))
plot(reg)
box()
...
\clearpage	
<<nbobs, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, results="asis">>=
	
	
	cap<-"Number of observations per group and per season."
	res<-ddply(d,.(Group,MonthC),nrow)
	res<-res[res$Group!="",]
	tab<-xtable(res,caption=cap)
	print(tab,include.rownames=FALSE,caption.placement="top")
	
@

\clearpage
<<seasonal_presence, eval=TRUE, cache=TRUE, echo=FALSE, message=FALSE, warning=FALSE, results="asis">>=
	
	
	cap<-""
	n<-1:3
	ds<-SpatialPoints(cbind(d$LongStart,d$LatStart),CRS(ll))
	ds<-spTransform(ds,CRS(proj4string(reg)))
	...
	for(i in n){
	  ...
	  x<-x[,.(dens=(sum(Count,na.rm=TRUE)/unique(month_effort))),by=.(Group,Month)]
	
   res<-x

   res<-merge(expand.grid(Group=unique(res$Group),Month=unique(res$Month),stringsAsFactors=FALSE),res,all.x=TRUE)
   ...
   res<-res[order(res$Group,res$Month),]
   res<-res[res$Group!="",]
   ...
   subplot({plot(reg,col=ifelse(is.na(match(n,n[i])),NA,"grey85"),border="grey85",lwd=1)},pars=list(mar=c(0,0,0,0)),x=par("usr")[1:2],y=par("usr")[3:4],hadj=0,vadj=0,inset=c(0.1,0.1),size=c(2,2))
   
   invisible(sapply(seq_along(g),function(k){
     x<-res[res$Group==g[k],]
   	 lines(f(x$dens),col=brewer.pal(length(g),"Paired")[k],lwd=5)
   }))
   ...
   legend("topleft",inset=c(0,-0.15),legend=g,lwd=5,col=brewer.pal(length(g),"Paired"),horiz=FALSE,ncol=3,bty="n",xpd=TRUE)
