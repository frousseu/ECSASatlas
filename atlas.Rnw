...
<<region_atlantic, echo=FALSE, message=FALSE, warning=FALSE>>=
...
<<nbobs, echo=FALSE, message=FALSE, warning=FALSE, results="asis">>=
...
<<seasonal_presence, echo=FALSE, message=FALSE, warning=FALSE, results="asis">>=
 ...
 g<-unique(res$Group)
 options(scipen=20)
 plot(as.numeric(res$Month),res$dens,type="n",xaxt="n",las=2,log="y")
 sapply(seq_along(g),function(i){
   x<-res[res$Group==g[i],]
 	 lines(x$dens,col=brewer.pal(length(g),"Paired")[i],lwd=6)
 })
 axis(1,at=seq_along(levels(res$Month)),label=levels(res$Month))
 legend("top",legend=g,lwd=6,col=brewer.pal(length(g),"Paired"),bty="n",horiz=TRUE)
